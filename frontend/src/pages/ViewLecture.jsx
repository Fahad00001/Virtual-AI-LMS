...
const ViewLecture = () => {
    ...
    return (
        <div className='min-h-screen bg-gray-50 p-6 flex flex-col md:flex-row gap-6'>
            {/* left area  ot top */}
            <div className='w-full md:w-1/2 bg-white rounded-2xl shadow-md p-6 border  border-gray-200'>
                <div className='mb-6'>
                    <h2 className='text-2xl font-bold items-center flex justify-start  gap-[20px] text-gray-800'> <FaArrowLeft onClick={() => navigate("/")} className='text-black w-[22px] h-[22px] cursor-pointer' /> {selectedCourse?.title}</h2>
            ...
            <div className="w-full md:w-1/3 bg-white rounded-2xl shadow-md p-6 border border-gray-200 h-fit">
                ...
                <div className='flex flex-col gap-3 mb-6'>
                    {selectedCourse?.lectures?.length > 0 ?
                        ...
                        (selectedCourse?.lectures?.map((lecture, index) => (
                            <button onClick={()=>setSelectedLecture(lecture)} className={`flex items-center justify-between  p-3 rounded-lg  border transition text-left ${selectedLecture?._id===lecture._id?'bg-gray-200 border-gray-500':'hover:bg-gray-50 border-gray-300'}`} key={index} >
                ...
                {/* educator info  */}
                 {creatorData&& <div className='mt-4 border-t pt-5'>

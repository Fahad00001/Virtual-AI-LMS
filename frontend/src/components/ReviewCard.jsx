...
const ReviewCard = ({comment,rating,photoUrl,name,description,courseTitle}) => {
  return (
    <div className='bg-white p-6 rounded-xl shadow-md  hover:shadow-lg transition-all duration-300 max-w-sm w-full'>
        <div className='flex items-start mb-3 text-yellow-400 text-sm '>
            {
        ...
        <div className='flex items-center  gap-2'>
            <img src={photoUrl} className='h-10 w-10 rounded-full object-cover' alt="" />
           <div>
           <h2 className='font-semibold text-gray-600 text-sm'>{name}</h2>
